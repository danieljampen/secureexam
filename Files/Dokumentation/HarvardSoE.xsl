	...
	<xsl:template name="Start">
		<xsl:choose>
			<xsl:when test="b:Version">
				<xsl:text>2014.12.13</xsl:text>
			</xsl:when>
			<xsl:when test="b:StyleName">
				<xsl:text>Harvard – SoE</xsl:text>
			</xsl:when>
			<xsl:when test="b:category">
				<xsl:text>Edited Office Online</xsl:text>
			</xsl:when>
					...
					<xsl:for-each select="b:Author/b:Editor/b:NameList/b:Person">
						...
						<xsl:choose>
							<xsl:when test="$cEditorLastName=1">
								<xsl:value-of select="b:Last"/>
								<xsl:choose>
									<xsl:when test ="$cEditorMiddleName=0 and $cEditorFirstName=0 and $cEditor>1 and (position()+1)=$cEditor">
										<xsl:call-template name ="templ_prop_Space"/>
										<xsl:call-template name ="templ_prop_BeforeLastAuthor"/>
										<xsl:call-template name ="templ_prop_Space"/>
									</xsl:when>
									<xsl:when test ="$cEditorMiddleName=0 and $cEditorFirstName=0">
										<xsl:call-template name ="templ_prop_Space"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name ="templ_prop_ListSeparator"/>
						...
						<xsl:choose>
							<xsl:when test="$cEditorFirstName=1">
								...
								<xsl:choose>
									...
									<xsl:when test ="$cEditorMiddleName=0 and position()!=$cEditor">
										<xsl:value-of select="' / '"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name ="templ_prop_Space"/>
					...
					<xsl:choose>
						<xsl:when test="$cAuthorLastName=1">
							...
							<xsl:choose>
								...
								<xsl:when test ="$cAuthorMiddleName=0 and $cAuthorFirstName=0">
										<xsl:call-template name ="templ_prop_Space"/>
									</xsl:when>
								<xsl:otherwise>
									<xsl:call-template name ="templ_prop_ListSeparator"/>
